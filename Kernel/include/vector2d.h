...
namespace venus
{
	namespace core
	{
		...
		/** As of Venus 1.6, this class supercedes position2d, which should
		...
		public:
			...
			f64 getAngleTrig() const
			{
				...
				if (Y > 0)
					if (X > 0)
						return atan((venus::f64)Y / (venus::f64)X) * RADTODEG64;
					else
						return 180.0 - atan((venus::f64)Y / -(venus::f64)X) * RADTODEG64;
				else
					if (X > 0)
						return 360.0 - atan(-(venus::f64)Y / (venus::f64)X) * RADTODEG64;
					else
						return 180.0 + atan(-(venus::f64)Y / -(venus::f64)X) * RADTODEG64;
			}
